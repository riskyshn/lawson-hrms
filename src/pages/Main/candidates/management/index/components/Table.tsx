...
const Table: React.FC<PropTypes> = () => {
  ...
  const bodyItems = candidates.map((candidate, i) => ({
    items: [
        ...
        children: (
          <div className="flex gap-3">
            ...
            <div>
              ...
              <span className="rounded-lg bg-green-100 px-2 py-1 text-xs font-semibold text-success-600">{candidate.match}% Match</span>
            </div>
          </div>
        ),
      },
      ...
      { children: candidate.applyDate, className: 'text-center' }, // Display formatted date
  ...
  return (
    <>
      ...
      {showOptionModal && selectedCandidate && modalType === 'MoveAnotherVacancy' && (
        <MoveAnotherVacancyModal show={showOptionModal} onClose={() => setShowOptionModal(false)} candidate={selectedCandidate} />
      )}

      {/* Render ProcessModal if modalType is 'Process' */}
      {showOptionModal && selectedCandidate && modalType === 'Process' && (
        <ProcessModal show={showOptionModal} onClose={() => setShowOptionModal(false)} candidate={selectedCandidate} />
      )}

      {/* Render ViewHistoryModal if modalType is 'View History' */}
      {showOptionModal && selectedCandidate && modalType === 'ViewHistory' && (
        <ViewHistoryModal show={showOptionModal} onClose={() => setShowOptionModal(false)} candidate={selectedCandidate} />
      )}
    </>
  )
}
