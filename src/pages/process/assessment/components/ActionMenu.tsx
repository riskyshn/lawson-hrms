...
const ActionMenu: React.FC<MenuListProps> = ({ options, items }) => {
  ...
  const handleViewDetails = (option: string, candidateId: string) => {
    if (option === 'Move to Another Vacancy') {
      setModalType('MoveAnotherVacancy')
      setShowOptionModal(true)
    }
    if (option === 'Process') {
      navigate(`/process/assessment/${candidateId}/process`)
    }
    if (option === 'Offering Letter') {
      navigate(`/process/assessment/${candidateId}/offering-letter`)
    }
            ...
            <Menu.Item key={index}>
              {({ active }) => (
                <button
                  ...
                  onClick={() => handleViewDetails(option, items.id)}
                >
