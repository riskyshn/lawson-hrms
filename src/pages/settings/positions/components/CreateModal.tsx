...
const CreateModal: React.FC<CreateModalProps> = ({ onClose, onCreated, show }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const createdItem = await organizationService.createPosition(data)
      onCreated?.(createdItem)
      toast('Position created successfully', { color: 'success' })
  ...
  return (
    <Modal as="form" onSubmit={onSubmit} show={show}>
      <ModalHeader onClose={onClose} subTitle="Set up a new position for your company">
        Create Position
      </ModalHeader>
