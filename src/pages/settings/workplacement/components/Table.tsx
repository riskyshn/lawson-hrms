...
const Table: React.FC<TableProps> = ({ items, onSubmitSuccess }) => {
  const headerItems = [
    { children: 'Work Placement', className: 'text-left' },
    // { children: 'Employees' },
    { children: 'Action', className: 'w-24' },
  ]

  const bodyItems = items.map((workPlacement) => ({
    items: [
      {
        children: (
          <>
            <span className="block font-semibold">{workPlacement.name}</span>
          </>
        ),
      },
      // {
      ...
      // },
      {
        children: <ActionMenu items={workPlacement} onSubmitSuccess={onSubmitSuccess} />,
      },
    ],
  }))
