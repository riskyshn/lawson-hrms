...
const EditModal: React.FC<PropType> = ({ item, onClose, onUpdated }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      await payrollService.updateDeductionComponent(item.oid, { ...data, maxCap: currencyToNumber(data.maxCap) })
      toast('Deduction component updated successfully.', { color: 'success' })
      onUpdated?.()
      onClose?.()
      setTimeout(() => {
        reset()
