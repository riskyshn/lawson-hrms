 ...
 */
const replaceDecimalSeparator = (value: string, decimalSeparator: FormatValueOptions['decimalSeparator'], isNegative: boolean): string => {
  let newValue = value
...
const replaceParts = (
  ...
  }: Pick<FormatValueOptions, 'prefix' | 'groupSeparator' | 'decimalSeparator' | 'decimalScale' | 'disableGroupSeparators'>,
): string => {
  return parts
    .reduce(
      (prev, { type, value }, i) => {
